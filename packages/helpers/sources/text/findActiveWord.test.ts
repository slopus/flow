...
import { findActiveWord, findActiveWordString, getActiveWordQuery } from './findActiveWord.js';
...
describe('findActiveWord', () => {
    describe('basic prefix detection', () => {
        it('should detect @ mention at cursor', () => {
            ...
            expect(result).toEqual({ word: '@john', offset: 6, length: 5, activeWord: '@john', activeLength: 5, endOffset: 11 });
        });
        ...
        it('should detect : emoji at cursor', () => {
            ...
            const result = findActiveWord(content, selection);
            expect(result).toEqual({ word: ':happy', offset: 7, length: 6, activeWord: ':happy', activeLength: 6, endOffset: 13 });
        });
        ...
        it('should detect / command at cursor', () => {
            ...
            const result = findActiveWord(content, selection);
            expect(result).toEqual({ word: '/help', offset: 5, length: 5, activeWord: '/help', activeLength: 5, endOffset: 10 });
        });
        ...
        it('should detect # tag at cursor', () => {
            ...
            const result = findActiveWord(content, selection, ['@', ':', '/', '#']);
            expect(result).toEqual({ word: '#important', offset: 8, length: 10, activeWord: '#important', activeLength: 10, endOffset: 18 });
        });
        ...
        it('should return just the prefix when typed alone', () => {
            ...
            const result = findActiveWord(content, selection);
            expect(result).toEqual({ word: '@', offset: 6, length: 1, activeWord: '@', activeLength: 1, endOffset: 7 });
        });
    });
    ...
    describe('word boundary detection', () => {
        ...
        it('should detect prefix after space', () => {
            ...
            const selection = { start: 11, end: 11 };
            const result = findActiveWord(content, selection);
            expect(result).toEqual({ word: '@user', offset: 6, length: 5, activeWord: '@user', activeLength: 5, endOffset: 11 });
        });
        ...
        it('should detect prefix at start of line', () => {
            ...
            const result = findActiveWord(content, selection);
            expect(result).toEqual({ word: '@user', offset: 0, length: 5, activeWord: '@user', activeLength: 5, endOffset: 5 });
        });
    ...
    describe('stop character handling', () => {
        ...
        it('should stop at comma', () => {
            ...
            const result = findActiveWord(content, selection);
            expect(result).toEqual({ word: '@user', offset: 4, length: 5, activeWord: '@user', activeLength: 5, endOffset: 9 });
        });
        ...
        it('should stop at parentheses', () => {
            ...
            const result = findActiveWord(content, selection);
            expect(result).toEqual({ word: '@user', offset: 1, length: 5, activeWord: '@user', activeLength: 5, endOffset: 6 });
        });
        ...
        it('should stop at semicolon', () => {
            ...
            const selection = { start: 10, end: 10 };
            const result = findActiveWord(content, selection);
            expect(result).toEqual({ word: '@user', offset: 5, length: 5, activeWord: '@user', activeLength: 5, endOffset: 10 });
        });
    });
    ...
    describe('multiple space handling', () => {
        ...
        it('should stop at multiple spaces', () => {
            ...
            const result = findActiveWord(content, selection);
            expect(result).toEqual({ word: '@user', offset: 7, length: 5, activeWord: '@user', activeLength: 5, endOffset: 12 });
        });
    ...
    describe('custom prefixes', () => {
        ...
        it('should work with multiple custom prefixes', () => {
            ...
            expect(result1).toEqual({ word: '$user', offset: 6, length: 5, activeWord: '$user', activeLength: 5, endOffset: 11 });

            const content2 = 'Hello %task';
            const selection2 = { start: 11, end: 11 };
            const result2 = findActiveWord(content2, selection2, ['$', '%']);
            expect(result2).toEqual({ word: '%task', offset: 6, length: 5, activeWord: '%task', activeLength: 5, endOffset: 11 });
        });
    ...
    describe('complex scenarios', () => {
        it('should handle multiple prefixes in same line', () => {
            ...
            expect(result1).toEqual({ word: '@john', offset: 4, length: 5, activeWord: '@john', activeLength: 5, endOffset: 9 });

            const selection2 = { start: 22, end: 22 };
            const result2 = findActiveWord(content, selection2);
            expect(result2).toEqual({ word: ':smile:', offset: 15, length: 7, activeWord: ':smile:', activeLength: 7, endOffset: 22 });

            const selection3 = { start: 32, end: 32 };
            const result3 = findActiveWord(content, selection3);
            expect(result3).toEqual({ word: '/help', offset: 27, length: 5, activeWord: '/help', activeLength: 5, endOffset: 32 });
        });
        ...
        it('should handle long active words', () => {
            ...
            const result = findActiveWord(content, selection);
            expect(result).toEqual({ word: '@very_long_username_here', offset: 6, length: 24, activeWord: '@very_long_username_here', activeLength: 24, endOffset: 30 });
        });
