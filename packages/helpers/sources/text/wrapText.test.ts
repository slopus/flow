...
describe('wrapText', () => {
    describe('basic wrapping', () => {
        test('wraps text at word boundaries', () => {
            expect(wrapText('hello world', 10)).toBe('hello\nworld');
        });
        ...
        test('handles single word longer than width', () => {
            expect(wrapText('verylongword', 5, { hard: false })).toBe('verylongword');
        });
    });
    ...
    describe('hard wrapping', () => {
        test('does NOT hard wrap by default (hard: false is default)', () => {
            ...
            expect(wrapText('verylongword', 5)).toBe('verylongword');
            expect(wrapText('verylongword', 5, {})).toBe('verylongword');
            expect(wrapText('verylongword', 5, { hard: false })).toBe('verylongword');
        });

        test('breaks long words with hard: true', () => {
            expect(wrapText('verylongword', 5, { hard: true })).toBe('veryl\nongwo\nrd');
        });
        ...
        test('hard wraps mixed content', () => {
            ...
            expect(wrapText('short verylongword', 10, { hard: true })).toBe('short very\nlongword');
        });

        test('hard wraps exactly at boundary', () => {
            expect(wrapText('12345678901234', 7, { hard: true })).toBe('1234567\n8901234');
        });

        test('hard wraps wide characters correctly', () => {
            // 世界世界世界 = 10 columns (5 chars × 2 width each)
            expect(wrapText('世界世界世界', 6, { hard: true })).toBe('世界世\n界世界');
        });

        test('hard wraps emojis correctly', () => {
            expect(wrapText('🎉🎉🎉🎉🎉', 6, { hard: true })).toBe('🎉🎉🎉\n🎉🎉');
        });
    ...
    describe('wide characters', () => {
        test('handles CJK characters (width 2)', () => {
            expect(wrapText('hello 世界', 8)).toBe('hello\n世界');
        });
    ...
    describe('ANSI codes', () => {
        ...
        test('handles color codes with zero width', () => {
            ...
            expect(wrapText(input, 10)).toBe('\x1b[31m\x1b[39m');
        });
    ...
    describe('edge cases', () => {
        ...
        test('handles very long lines', () => {
            ...
            const output = wrapText(long, 10, { hard: true });
    ...
    describe('unicode normalization', () => {
        test('normalizes unicode characters', () => {
            ...
            expect(wrapText(composed, 10)).toBe(wrapText(decomposed, 10));
        });
    });
    ...
    describe('complex scenarios', () => {
        test('handles mixed content with wrapping', () => {
            ...
            const output = wrapText(input, 20);
            const lines = output.split('\n');
        ...
        test('handles ANSI codes with hard wrapping', () => {
            ...
            const output = wrapText(input, 8, { hard: true });
            expect(output).toContain('\x1b[31m');
            expect(output).toContain('\x1b[39m');
        ...
        test('preserves formatting across multiple wraps', () => {
            ...
            const output = wrapText(input, 15, { hard: false });
            const lines = output.split('\n');
    ...
    describe('option combinations', () => {
        test('trim: false, hard: true', () => {
            const output = wrapText('  verylongword  ', 8, { trim: false, hard: true });
            expect(output).toContain('  ');
        });

        test('wordWrap: false, trim: true', () => {
            const output = wrapText('  hello world  ', 8, { wordWrap: false, trim: true });
            expect(output).toBe('hello wo\nrld');
        });

        test('all options enabled', () => {
            const output = wrapText('  hello verylongword world  ', 10, {
                trim: true,
                hard: true,
                wordWrap: true,
            });
